...
const FIXTURE_HOME_TEMPLATE = resolve(
...
async function createFixtureHome() {
  const homeDir = await mkdtemp(join(tmpdir(), "hub-run-fixture-home-"));
  await cp(FIXTURE_HOME_TEMPLATE, homeDir, {
    recursive: true,
    filter: (source) =>
      !source.includes("/.codex/tmp/") &&
      !source.includes("/.claude/statsig/"),
  });
  return {
    homeDir,
    stateDbPath: resolve(homeDir, ".codex/state_5.sqlite"),
  };
}

async function withFixtureApp<T>(
  run: (props: {
  ...
  try {
    return await run({
      app: createFixtureApp(fixture.homeDir),
...
function insertFixtureThread(stateDbPath: string, options: {
  id: string;
  source: string;
  ...
  return () => {
    const cleanupDatabase = new DatabaseSync(stateDbPath);
    try {
...
async function createFixtureCodexSessionFile(homeDir: string, options: {
  id: string;
  lines: string[];
...
test("codex sessions route hides subagent threads from session list", async () => {
  await withFixtureApp(async ({ app, stateDbPath }) => {
    const cleanup = insertFixtureThread(stateDbPath, {
      id: "codex-session-2",
      source:
...
test("codex sessions route hides sessions without meaningful user title", async () => {
  await withFixtureApp(async ({ app, homeDir }) => {
    const cleanup = await createFixtureCodexSessionFile(homeDir, {
...
test("codex message route filters by mode before pagination", async () => {
  await withFixtureApp(async ({ app }) => {
    const cookie = await login(app);

    const latestResponse = await app.request(
      "/api/providers/codex/sessions/codex-session-1/messages?limit=1&mode=compact",
      {
        headers: { cookie },
      },
    );
    ...
    const latestPayload = await latestResponse.json();
    assert.deepEqual(
      latestPayload.messages.map((message: { kind: string; role: string; text: string }) => ({
        kind: message.kind,
        role: message.role,
        text: message.text,
      })),
    ...
    const olderResponse = await app.request(
      "/api/providers/codex/sessions/codex-session-1/messages?limit=1&before=1&mode=compact",
      {
        headers: { cookie },
      },
    );
    ...
    const olderPayload = await olderResponse.json();
    assert.deepEqual(
      olderPayload.messages.map((message: { kind: string; role: string; text: string }) => ({
        kind: message.kind,
        role: message.role,
        text: message.text,
      })),
...
test("message route rejects invalid mode", async () => {
  await withFixtureApp(async ({ app }) => {
    const cookie = await login(app);
    const response = await app.request(
      "/api/providers/codex/sessions/codex-session-1/messages?mode=weird",
      { headers: { cookie } },
    );
